...
class StreamWriter(BufferedWriter):
    ...
    def _flush_unlocked(self):
        ...
        to a TLS-only port, OpenSSL detects invalid pre-handshake data and may
        invalidate the underlying socket. At that point, the socket
        may or may not be writable at the moment of flush; writes
        (e.g., sending a 400 Bad Request) may succeed or raise OSError.
        This override swallows OSError and clears the buffer
